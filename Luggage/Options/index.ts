...
export namespace Options {
	export function is(value: Options | any): value is Options {
		return (
			...
			(value.flights == undefined || (Array.isArray(value.flights) && value.flights.every((f: any) => Flights.is(f))))
		)
	}
	...
	export function filter(booking: Options, passenger: Passenger): (Options | Luggage.Category)[] {
